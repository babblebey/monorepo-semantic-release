...
const CORE_DEFAULT_TAG_FORMAT = "v${version}";
...
function createCompatibleLogger(baseLogger, fallbackScopeName = "semantic-release") {
  const compatibleLogger = {
    ...
    start: (...args) => baseLogger.start(...args),
    skip: (...args) => baseLogger.skip(...args),
    complete: (...args) => baseLogger.complete(...args),
    await: (...args) => baseLogger.await(...args),
    scope: (...scopes) => {
...
function resolvePackageTagFormat({ options, pkg, resolvedConfigOptions }) {
  if (
    resolvedConfigOptions.tagFormat === CORE_DEFAULT_TAG_FORMAT &&
    pkg.tagFormat === undefined &&
    options.runtimeOverrides.tagFormat === undefined
  ) {
    return options.defaultTagFormat;
  }

  return resolvedConfigOptions.tagFormat;
}
...
export default async function buildPlan({ sharedContext, packages, options }) {
  ...
  for (const pkg of packages) {
    ...
    packageContext.logger.start("Starting release plan...");
    ...
    const tagFormat = resolvePackageTagFormat({ options, pkg, resolvedConfigOptions });
    const resolvedOptions = {
      ...resolvedConfigOptions,
      tagFormat: applyTagFormatTemplate(tagFormat, pkg.name),
    };
    ...
    if (planned.skipped) {
      packageContext.logger.complete(`Completed release plan (skipped: ${planned.skipped}).`);
    } else if (planned.changed) {
      packageContext.logger.complete("Completed release plan (release required).");
    } else {
      packageContext.logger.complete("Completed release plan (no release required).");
    }
